...
Se recomienda ajustar los parametros de memoria en neo4j.properties (/usr/local/bin/neo4j/conf)
...
https://www.dropbox.com/s/sz0ggt29eclpaq9/graph.db.Onto.gz  

*descomprimir con:*  `tar zxvf graph.db.Onto.gz ` 


*ajustar parametro de ubicacion de la base de datos a la carpeta contenedora  org.neo4j.server.database.location=  en:  
`neo4j-server.properties`  (/usr/local/bin/neo4j/conf)
...
Una de las motivaciones al realizar este trabajo se centraba en la posibilidad de tranformar la información impresa en expersión gráfica que acusara las relaciones que regularmente se desconocen o resultan poco visibles. Esto esta cubierto: Se tranformaron 3 clases  ontologicas de DBPEDIA en un gran red multicapa y multiclase que contiene mas de 15 millones de elementos (nodos, aristas, relaciones, propiedades, etiquetas). La consulta a esta base de datos es ágil basada en índices. 
...
No obstante lo anterior, la recomendación para hacer análisis de grafos en gran escala consiste en aplicar una consulta a Neo4j vía shell, recuperarla en un archivo y transformarlo para su carga en la herramienta expecífica. 
