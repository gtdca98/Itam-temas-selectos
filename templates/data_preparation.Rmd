...
head(ds)           
tail(ds)
# Si se usa dplyr, no son necesarios los cuatro pasos anteriores, solo con la impresión a pantalla del ds
# bastaría
ds[sample(nrow(ds), 6)]
str(ds)
summary(ds)
...
# Otra alternativa es eliminar los que tienen más del 70% de faltantes
# No recomendado sin hacer análisis de missings
# mvn <- names(which(mvc >= 0.7*nrow(ds)))
# ignore <- union(ignore, mvn)


# Ignoramos las variables con muchos niveles
# Por ejemplo el randomForest no puede con variables categóricas de más de 32 niveles.
# Es más recomendable recodificar en menos niveles
...
# Es mucho sabio tener una función de imputación especial
# No se recomienda imputar sin saber el comportamiento de los missings
